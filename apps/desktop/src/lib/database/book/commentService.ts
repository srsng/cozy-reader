...
export class CommentService {
	 ...
	 * 查询评论列表
	 */
	static async getComments(options: CommentQueryOptions = {}): Promise<DatabaseResult<Comment[]>> {
		return await DatabaseUtils.safeExecute(async () => {
			const db = await getDatabase();
	...
	static async getCommentStatistics(): Promise<DatabaseResult<CommentStatistics>> {
		return await DatabaseUtils.safeExecute(async () => {
			...
			const averageCommentsPerBook = uniqueBookCount > 0 ? totalComments / uniqueBookCount : 0;
