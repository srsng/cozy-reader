...
export class DatabaseUtils extends BaseUtils {
	...
	> {
		return await this.safeExecute(async () => {
			...
			if (options.endTimeoutSessions !== false) {
				...
				endedTimeoutSessions = await ReadingSessionService.endTimeoutSessions(timeoutMinutes);
			}
	...
	static async resetDatabase(): Promise<DatabaseResult<void>> {
		return await this.safeExecute(async () => {
			...
			// 重置自增 ID
			await db.execute('DELETE FROM sqlite_sequence WHERE name IN ("books", "reading_sessions")');
